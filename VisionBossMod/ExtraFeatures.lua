...
function VBM_ExtraFeatures_OnLoad()
	...
	this:RegisterEvent("LFG_PROPOSAL_SUCCEEDED");
	this:RegisterEvent("LFG_PROPOSAL_FAILED");
end

function VBM_ExtraFeatures_OnEvent()
	...
	if(event == "CHAT_MSG_RAID" or
		...
		VBM_AbortPullCD(arg1);
		VBM_EasterEggs(arg1);
		return;
	end
	
	if(event == "CHAT_MSG_LOOT") then
		VBM_BadgeLootReminder(arg1);
		VBM_Brewfest(arg1);
		return;
	end
	
	if(event == "LOOT_BIND_CONFIRM") then
		SoloAutoLootBoPConfirm(arg1);
		return;
	end
	if(event == "LOOT_SLOT_CLEARED") then
		SoloAutoLootBoPCleared(arg1);
		return;
	end
	if(event == "LOOT_OPENED") then
		SoloAutoLootBoPOpen(arg1);
		return;
	end
...
function VBM_Officers_chat_commands()
	if(arg1 == "!a") then
...
function VBM_SSDI_Alert()
	...
	if(combatEvent == "SPELL_AURA_APPLIED" and destName) then
		...
		if(spellName == "Soulstone Resurrection") then
			vbm_send_synced("GAINSS "..destName,true);
		elseif(spellName == "Divine Intervention") then
			vbm_send_synced("GAINDI "..destName,true);
		end
	end
end

function VBM_SSDI_SS(who)
	if(VBM_GetS("SSDIAlert")) then
		vbm_infowarn(who.." gains Soulstone",5,0,1,0);
		vbm_printc("|cFFFFFFFF"..who.." |cFF8888CCgains |cFFFFFFFFSoulstone");
	end
end

function VBM_SSDI_DI(who)
	if(VBM_GetS("SSDIAlert")) then
		vbm_infowarn(who.." gains Divine Intervention",5,0,1,0);
		vbm_printc("|cFFFFFFFF"..who.." |cFF8888CCgains |cFFFFFFFFDivine Intervention");
	end
end
...
function VBM_AutoRollLootSelect()
...
function VBM_MasterLootReminder(status)
	...
	if(VBM_GetS("MasterLootReminder") and GetNumRaidMembers() > 20) then
...
function VBM_BuffAlerter()
	local combatEvent, sourceName, destName = arg2,arg4,arg7;
	--you gain a buff
	if(combatEvent == "SPELL_AURA_APPLIED" and destName and destName == UnitName("player")) then
		local spellName, auraType = arg10,arg12;
		if(auraType == "BUFF") then
			local text;
			local up = false;
			
			for t,u in pairs(VBM_BUFF_ALERTER_DATA) do
				if(spellName==t) then
					text = t;
					if(u==1) then
						up = true;
...
function VBM_CCBigBrother()
	local combatEvent, sourceName, sourceFlags, destName, destFlags = arg2,arg4,arg5,arg7,arg8;

	-- if aura is dispelled on an npc
	if( (combatEvent == "SPELL_AURA_BROKEN" or combatEvent == "SPELL_AURA_BROKEN_SPELL") and destName) then
		...
		local extraSpellName, spellName, auraType = arg10,arg13,arg15;
		--fix auto attack
		if(not spellName) then spellName = "Auto Attack"; end
		--check for cc
		if(VBM_CCBBSpells[extraSpellName]) then
			vbm_print(vbm_c_p..">>>"..vbm_c_w..extraSpellName..vbm_c_p.."<<< ["..CombatLog_String_GetIcon(destFlags, "dest")..vbm_c_grey..destName..vbm_c_p.."]"..
						VBM_FlagsColor(destFlags).." removed by "..CombatLog_String_GetIcon(sourceFlags, "source")..vbm_c_grey..sourceName..vbm_c_p.."'s "..vbm_c_w..spellName);
		end
	end
end

function VBM_InterruptWatcher()
	local combatEvent, sourceName, sourceFlags, destName, destFlags = arg2,arg4,arg5,arg7,arg8;
	...
	if(combatEvent == "SPELL_INTERRUPT" and destName) then
		...
		local sflag = VBM_FlagsColor(sourceFlags);
		if(not sflag) then sflag = ""; end
		vbm_print(vbm_c_p.."["..CombatLog_String_GetIcon(sourceFlags, "source")..vbm_c_grey..sourceName..vbm_c_p.."] "..vbm_c_w..spellName..
					sflag.." INTERRUPT "..vbm_c_p.."["..CombatLog_String_GetIcon(destFlags, "dest")..vbm_c_grey..destName..vbm_c_p.."] "..vbm_c_w..extraSpellName);
					
		if(VBM_GetS("InterruptWatcherAnnounce") and sourceName and sourceName == VBM_YOU) then
			vbm_say(""..spellName.." - >>"..extraSpellName.."<<");
		end
	end	
end

function VBM_MisdirectionWatcher()
	local combatEvent, sourceName, sourceFlags, destName, destFlags = arg2,arg4,arg5,arg7,arg8;
...
function VBM_TricksoftheTradeWatcher()
	local combatEvent, sourceName, sourceFlags, destName, destFlags = arg2,arg4,arg5,arg7,arg8;
