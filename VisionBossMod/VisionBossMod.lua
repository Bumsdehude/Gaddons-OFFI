--[[
local timestamp, combatEvent, sourceGUID, sourceName, sourceFlags, destGUID, destName, destFlags = arg1,arg2,arg3,arg4,arg5,arg6,arg7,arg8;
...
--boss handling vars
VBM_DUNGEON_DIFFICULTY = nil; -- will be 1 for normal or 2 for heroic
VBM_DUNGEON_SIZE = nil; -- will be 10 or 25
VBM_ZONE = false;
VBM_BOSS = false;
VBM_BOSSSTART = {};
...
function VisionBossMod_OnLoad()
	...
	this:RegisterEvent("UNIT_COMBAT"); -- used to detect then encounter starts
	this:RegisterEvent("UNIT_TARGET"); -- used to detect then encounter starts
	this:RegisterEvent("PLAYER_TARGET_CHANGED"); --used to detect bosses
	
	this:RegisterEvent("COMBAT_LOG_EVENT_UNFILTERED"); --used to detect then bosses die
	
	this:RegisterEvent("GUILD_MOTD");
	--instance difficulty tracking
	this:RegisterEvent("PLAYER_DIFFICULTY_CHANGED");
	this:RegisterEvent("UPDATE_INSTANCE_INFO");
end


function VisionBossMod_OnEvent(event)
	if(VBM_ZONE and event=="COMBAT_LOG_EVENT_UNFILTERED" and arg2 == "UNIT_DIED") then
		VBM_DetectBossDeath();
		return;
	end
	...
	if(event == "GUILD_MOTD") then
		if(gmotd_only_once) then
			if(not VBM_HAS_INIT or VBM_GetS("ShowGMOTD")) then
				vbm_infowarn("GMOTD: "..vbm_c_w..VBM_linebreakStr(arg1,45,true),20,0,1,0);
			end
			gmotd_only_once = false; --will also auto turn of after LOAD_LAG_DELAY sec after login
		end
		return;
	end
...
function VisionBossMod_OnUpdate()
	--dont update to fast
...
function VisionBossMod_Init()
	...
	--print load message 
	vbm_print("|cFF8888CCVisionBossMod v"..VBM_VERSION.." loaded! Type /vbm to show StatusFrame");
	...
	VBM_Delay(VBM_LOAD_LAG_TIME,function() gmotd_only_once = false; end); --to make sure gmotd are only displayed during login
...
function VBM_RequestUpdate()
		...
		--print msg
		vbm_print("|cFF8888CC<VisionBossMod> Manual update complete.");
end
...
function vbm_recive_mess(msg,from)
	...
	if(found) then
		--update version list
		VBM_VERSION_LIST[from] = tonumber(p1);
		--check for new version
		if(tonumber(p1) > VBM_VERSION and VBM_NEW_VERSION_OUT == false) then
...
function vbm_recive_synced(sync,msg,from)
	...
	-- Divine Intervention
	found,_,p1 = string.find(msg,"GAINDI (.+)");
	if(found) then
		VBM_SSDI_DI(p1);
		return;
	end
	-- Soulstone
	found,_,p1 = string.find(msg,"GAINSS (.+)");
	if(found) then
		VBM_SSDI_SS(p1);
		return;
	end
...
function VBM_DoZoneDetect()
	...
	if(VBM_Instaces[zone]) then
		...
		if(VBM_ZONE) then
			...
			VBM_AutoZoneOut();
			VBM_ZONE = false;
			VBM_DUNGEON_DIFFICULTY = nil;
			VBM_DUNGEON_SIZE = nil;
			--reset bossloading vars
			VBM_BOSS = false;
			VBM_BOSSSTART = {};
